...
public class ChatHeadService extends Service {			
	...
	private void handleStart(){
		...
		txt1 = (WebView) txtView.findViewById(R.id.txt1);
...
//		it.putExtra(Utils.EXTRA_MSG, sMsg);
//		startActivity(it);
	...
	private void showMsg(String sMsg){
		if(txtView != null && chatheadView != null) {
			...
			chatheadImg.setImageBitmap(faviconbm);
			txt1.loadUrl(sMsg);
			txt1.clearHistory();
			...
			if(isLeft){
				...
				txt_linearlayout.getLayoutParams().height =( 4 *szWindow.y) / 5;
				txt_linearlayout.getLayoutParams().width =  szWindow.x - 40 ;
			}else{
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if(intent != null){
			...
			if(sMsg != null && sMsg.length() > 0){
					try {
						URL url = new URL(bd.getString(Utils.FAV_URL));
						HttpURLConnection connection = (HttpURLConnection) url.openConnection();
						connection.setDoInput(true);
						connection.connect();
						InputStream inputStream = connection.getInputStream();
						 faviconbm = BitmapFactory.decodeStream(inputStream);
